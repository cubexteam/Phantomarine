...
class EncapsulatedPacket{
	...
	public static function fromInternalBinary(string $bytes, ?int &$offset = null) : EncapsulatedPacket{
		...
		$offset += 4;
		$packet->identifierACK = Binary::readInt(substr($bytes, $offset, 4));
		$offset += 4;
	...
	public function toInternalBinary() : string{
		return
			...
			Binary::writeInt($this->identifierACK ?? -1) .
	...
	public function getTotalLength() : int{
		return
			1 +
			2 +
			(PacketReliability::isReliable($this->reliability) ? 3 : 0) +
			(PacketReliability::isSequenced($this->reliability) ? 3 : 0) +
			(PacketReliability::isSequencedOrOrdered($this->reliability) ? 3 + 1 : 0) +
			($this->hasSplit ? 4 + 2 + 4 : 0) +
			strlen($this->buffer);
	}
