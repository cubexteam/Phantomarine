...
class Level implements ChunkManager, Metadatable{
	...
	public function updateAllLight(Vector3 $pos){
		if(($chunk = $this->getChunk($pos->x >> 4, $pos->z >> 4)) === null || $chunk->isLightPopulated() !== true){
			$this->server->getLogger()->debug("Skipped runtime light update of x={$pos->x},y={$pos->y},z={$pos->z} because the target area has not received base light calculation");
			return;
		}
	...
	public function setBlockAt(int $x, int $y, int $z, Block $block, bool $direct = false, bool $update = true) : bool{
		...
		$chunkX = $x >> 4;
		$chunkZ = $z >> 4;
		if(!$this->loadChunk($chunkX, $chunkZ, false)){
			throw new \InvalidArgumentException("Cannot set a block in un-generated terrain");
		}
	...
	public function useBreakOn(Vector3 $vector, Item &$item = null, Player $player = null, bool $createParticles = false) : bool{
		...
		if(!$this->isChunkLoaded($chunkX, $chunkZ) || !$this->isChunkGenerated($chunkX, $chunkZ)){
			return false;
		}
	...
	public function useItemOn(Vector3 $vector, Item &$item, int $face, float $fx = 0.0, float $fy = 0.0, float $fz = 0.0, Player $player = null) : bool{
		...
		$chunkX = $block->getFloorX() >> 4;
		$chunkZ = $block->getFloorZ() >> 4;
		if(!$this->isChunkLoaded($chunkX, $chunkZ) || !$this->isChunkGenerated($chunkX, $chunkZ)){
			return false;
		}
		...
		if($player !== null){
			...
			if(!$ev->isCancelled()){
				if(!$player->isSneaking()){
					...
					if($item->onActivate($player, $block, $target, $face, $fx, $fy, $fz)){
						return true;
					}
				}
                ...
                if(!$player->isSneaking() and $item->onActivate($player, $block, $target, $face, $fx, $fy, $fz)){
                    return true;
                }*/
			}else{
				return false;
	...
	public function getRandomChunk() : Chunk{
	...
	public function setChunk(int $chunkX, int $chunkZ, Chunk $chunk = null, bool $deleteEntitiesAndTiles = true){
		...
		unset($this->blockCollisionBoxCache[$index]);
		unset($this->chunkCache[$index]);
	...
	public function spawnXPOrb(Vector3 $pos, int $amount = 1){
		$orbs = [];
		...
		foreach(XPOrb::splitIntoOrbSizes($amount) as $split){
			$nbt = new CompoundTag("", [
				...
				new ListTag("Motion", [
					new DoubleTag("", (lcg_value() * 0.2 - 0.1) * 2),
					new DoubleTag("", lcg_value() * 0.4),
					new DoubleTag("", (lcg_value() * 0.2 - 0.1) * 2)
				]),
				new ListTag("Rotation", [
					new FloatTag("", lcg_value() * 360),
	...
	private function processChunkRequests() : void{
	...
	public function addEntity(Entity $entity){
		...
		if(array_key_exists($entity->getId(), $this->entities)){
			if($this->entities[$entity->getId()] === $entity){
				throw new \InvalidArgumentException("Entity " . $entity->getId() . " has already been added to this level");
			}else{
	...
	public function removeEntity(Entity $entity){
		...
		if(!array_key_exists($entity->getId(), $this->entities)){
			throw new \InvalidArgumentException("Entity is not tracked by this level (possibly already removed?)");
		}
	...
	public function addTile(Tile $tile){
		...
		if(!$tile->isValid() || $tile->getLevel() !== $this){
			throw new \InvalidArgumentException("Invalid Tile level");
		}
		if(!$this->isInWorld($tile->getFloorX(), $tile->getFloorY(), $tile->getFloorZ())){
			throw new \InvalidArgumentException("Tile position is outside the level bounds");
		}
