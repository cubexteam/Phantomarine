...
class Chunk{
	...
	public function addTile(Tile $tile){
		...
		$this->tileList[Chunk::blockHash($tile->x, $tile->y, $tile->z)] = $tile;
		$this->hasChanged = true;
	}
	public function removeTile(Tile $tile){
		unset($this->tiles[$tile->getId()]);
		unset($this->tileList[Chunk::blockHash($tile->x, $tile->y, $tile->z)]);
		$this->hasChanged = true;
	}
	...
	public function getTile(int $x, int $y, int $z){
		return $this->tileList[Chunk::blockHash($x, $y, $z)] ?? null;
	}
	...
	public function networkSerialize(?string $networkSerializedTiles) : string{
		...
		$extraData->putVarInt(count($this->extraData));
	...
	public static function fastDeserialize(string $data) : Chunk{
		...
		if($terrainGenerated){
			...
			if($lightPopulated){
				$unpackedHeightMap = unpack("v*", $stream->get(512));
