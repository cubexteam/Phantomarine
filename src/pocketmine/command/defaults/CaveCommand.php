...
class CaveCommand extends VanillaCommand{
	...
	public function caves(Position $pos, $cave, $tt = false){
		...
		if ($cave[3][0]) {
			...
			$l = $cave[4];
			if ($pitch < -10 && $pitch > -45 && $level->getBlockAt($s2[0], $s2[1] - 3, $s2[2])->getId() != 0) $this->tiankengy($level, $s2[0], $s2[1], $s2[2], $l / 2, 11, 0);
		}*/
	...
	public function explodeBlocks(Position $source, $rays = 16, $size = 4){
		...
		foreach($affectedBlocks as $block){
			if($block instanceof Block){
				$block->getLevel()->setBlock($block, BlockFactory::get(Block::AIR), false, false);
			}
		}
	}
	public function fdx($x, $y, $z, Level $level, $liu = false){
		...
		if($liu){
			$l = (mt_rand(0, 1) == 0) ? BlockFactory::get(Block::WATER) : BlockFactory::get(Block::LAVA);
			$i = mt_rand(3, 6);
	...
	public function tiankengy(Level $level, $x, $y, $z, $l, $id, $bd){
		if($level->getBlockAt($x, $y, $z)->getId() == 0) $level->setBlock(new Vector3($x, $y, $z), Item::get($id, $bd)->getBlock());
		if($l >= 0){
			...
			foreach($mz as $sss){
				switch($mz[$sss]){
					case 0:
						if($level->getBlockAt($x, $y, $z - 1)->getId() == 0) $this->tiankengy($level, $x, $y, $z - 1, $l - $random, $id, $bd);
						break;
					case 1:
						if($level->getBlockAt($x, $y, $z + 1)->getId() == 0) $this->tiankengy($level, $x, $y, $z + 1, $l - $random, $id, $bd);
						break;
					case 2:
						if($level->getBlockAt($x + 1, $y, $z)->getId() == 0) $this->tiankengy($level, $x + 1, $y, $z, $l - $random, $id, $bd);
						break;
					case 3:
						if($level->getBlockAt($x - 1, $y, $z)->getId() == 0) $this->tiankengy($level, $x - 1, $y, $z, $l - $random, $id, $bd);
						break;
