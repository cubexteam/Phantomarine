...
class Player extends Human implements CommandSender, ChunkLoader, IPlayer{
	...
	private const INCOMING_PACKET_BATCH_MAX_BUDGET = 100 * self::INCOMING_PACKET_BATCH_PER_TICK;
	...
	public function sleepOn(Vector3 $pos){
		...
		$pos = $pos->floor();
		$b = $this->level->getBlockAt($pos->x, $pos->y, $pos->z);
	...
	public function stopSleep(){
		if($this->sleeping instanceof Vector3){
			$b = $this->level->getBlockAt($this->sleeping->x, $this->sleeping->y, $this->sleeping->z);
			if($b instanceof Bed){
				$b->setOccupied(false);
			}
	...
	protected function handleMovement(Vector3 $newPos) : void{
		...
		if($distanceSquared > 225){
			 ...
			 */
			$this->server->getLogger()->debug($this->getName() . " moved too fast (" . sqrt($distanceSquared) . " blocks in 1 movement), reverting movement");
	...
	public function entityBaseTick($tickDiff = 1, $EnchantL = 0){
		...
		if($this->startActionDiff === 0){
			$this->setDataFlag(self::DATA_FLAGS, self::DATA_FLAG_ACTION, false);
			$this->startActionDiff = -1;
			$hasUpdate = true;
		}elseif($this->startActionDiff > 0 && $this->startAction === -1){
			$this->startActionDiff = max(0, $this->startActionDiff - $tickDiff);
		}
	...
	protected function processLogin(){
		...
		$ip = 0;
		foreach($this->server->getOnlinePlayers() as $p){
			if($this->getAddress() === $p->getAddress()){
				if(++$ip >= 4){
					$this->close($this->getLeaveMessage(),"С вашего IP-адреса уже играет максимально возможное количество игроков (3).");
					return;
				}
			}
		}
		...
		$yaw = $this->namedtag["Rotation"][0];
		$pitch = $this->namedtag["Rotation"][1];
		foreach([$yaw, $pitch] as $float){
			if(is_infinite($float) || is_nan($float)){
				$this->server->getLogger()->debug("Invalid saved data from " . $this->getName() . ", contains NAN/INF components, will be fixed");
				$this->namedtag["Rotation"][0] = $this->namedtag["Rotation"][1] = new FloatTag("", 0.0);
			}
		}
	...
	public function handleDataPacket(DataPacket $packet){
		...
		if($this->server->getNetwork()->isBlockedAddress($this->getAddress())){
			return;
		}
		...
		if($packet::NETWORK_ID === 0xfe){
			if($this->incomingPacketBatchBudget <= 0){
				...
				if($this->incomingPacketBatchBudget <= 0){
					throw new \RuntimeException("Receiving packets too fast");
				}
			}
		...
		switch($packet::NETWORK_ID){
			...
			case ProtocolInfo::LEVEL_SOUND_EVENT_PACKET:
				$sound = $packet->sound;
				if ($sound >= 0 and $sound <= 107 and $sound != 72){
			...
			case ProtocolInfo::MOVE_PLAYER_PACKET:
				...
				if($this->linkedEntity instanceof Entity){
					...
					/*if($entity instanceof Minecart){
						...
						$entity->motion->x = -sin($packet->yaw / 180 * M_PI);
						$entity->motion->z = cos($packet->yaw / 180 * M_PI);
					}*/
				}
				...
				if($packet->face >= 0 and $packet->face <= 5){
					...
					switch($item->getId()){
						case Item::FISHING_ROD:
							...
							$item->onClickAir($this, $directionVector);
					...
					if($entity instanceof Projectile and $entity->isAlive()){
						...
						$entity->spawnToAll();
						$this->level->broadcastLevelSoundEvent($this, LevelSoundEventPacket::SOUND_THROW, 319);
					}

					if($item instanceof Food && !$this->isSurvival()){
						break;
					}

					$this->setDataFlag(self::DATA_FLAGS, self::DATA_FLAG_ACTION);
					$this->startAction = $this->server->getTick();
					$this->startActionDiff = 10;
				}
				break;
					...
					case PlayerActionPacket::ACTION_START_BREAK:
						...
						if(!$ev->isCancelled()){
							...
							if($side instanceof Fire){
								$side->getLevel()->setBlock($side, BlockFactory::get(Block::AIR));
								break;
							}
						...
						if($this->startAction > -1 and $this->getDataFlag(self::DATA_FLAGS, self::DATA_FLAG_ACTION)){
							if($this->inventory->getItemInHand()->getId() === Item::BOW){
								...
								$inventory = match(true){
									$this->getOffHandInventory()->contains(Item::get(Item::ARROW, -1)) => $this->getOffHandInventory(),
									$this->getInventory()->contains(Item::get(Item::ARROW, -1)) => $this->getInventory(),
									default => null
								};

								if($this->isSurvival() and $inventory === null){
									$this->inventory->sendContents($this);
									$this->offhandInventory->sendContents($this);
									break;
								}elseif($this->isCreative() and $inventory === null){
									$inventory = $this->getInventory();
								}

								$arrow = null;

								$index = $inventory->first(Item::get(Item::ARROW, -1));
								...
								if($ev->isCancelled()){
									...
									if($this->isSurvival()){
										if(!$bow->hasEnchantment(Enchantment::TYPE_BOW_INFINITY)){
											...
											$projectile->setPickupMode(2);
										}
										if(($punchLevel = $bow->getEnchantmentLevel(Enchantment::TYPE_BOW_KNOCKBACK)) > 0){
											$projectile->setPunchKnockback($punchLevel);
										}
										if(($powerLevel = $bow->getEnchantmentLevel(Enchantment::TYPE_BOW_POWER)) > 0){
											$projectile->setBaseDamage($projectile->getBaseDamage() + (($powerLevel + 1) / 2));
										}
										if($bow->hasEnchantment(Enchantment::TYPE_BOW_FLAME)){
							...
							if($this->hasItemCooldown($this->inventory->getItemInHand())){
								$ev->setCancelled();
							}
							if($ev->isCancelled()){
								$this->inventory->sendContents($this);
			...
			case ProtocolInfo::REMOVE_BLOCK_PACKET:
				...
				$target = $this->level->getBlockAt($vector->x, $vector->y, $vector->z);
						...
						if($this->isSurvival()){
							...
							if($ev->getThornsDamage() > 0){
								...
								if(!$thornsEvent->isCancelled()){
									if($this->attack($thornsEvent) === true){
			...
			case ProtocolInfo::ANIMATE_PACKET:
				...
				if ($packet->action < 1 or $packet->action > 129){
					...
					break;
				}elseif($packet->action == AnimatePacket::ACTION_CRITICAL_HIT){
					break;
				}
			...
			case ProtocolInfo::ENTITY_EVENT_PACKET:
				...
				switch($packet->event){
					case EntityEventPacket::EATING_ITEM:
						...
						if($packet->data < 256 or $packet->data > 512){
							throw new \InvalidArgumentException("Garbage eating id passed {$packet->data}");
						}

						$data = $packet->data;

						if($packet->data !== ($expectId = $this->getInventory()->getItemInHand()->getId())){
							if ($data == 373){
								$packet->data = $expectId;
							}elseif($data == 374){
								$packet->data = $expectId;
							}elseif($data == 281){
								$packet->data = $expectId;
								...
								$this->server->getLogger()->debug("Player {$this->getName()} send invalid data in eating event expect {$expectId} passed {$packet->data}");
								return false;
							}
						}
					...
					case EntityEventPacket::USE_ITEM:
						...
						if($slot->canBeConsumed()){
							...
							$this->server->getPluginManager()->callEvent($ev);
							if(!$slot->canBeConsumedBy($this) or $this->hasItemCooldown($slot)){
								$ev->setCancelled();
							}
							if(!$ev->isCancelled()){
								$slot->onConsume($this);
			...
			case ProtocolInfo::COMMAND_STEP_PACKET:
				...
				if($packet->inputJson !== null){
					if(is_countable($packet->inputJson)){
						...
						if (isset($packet->inputJson["args"])) {
							$str = explode(" ", $packet->inputJson["args"], 20);
							if (count($str) > 10) {
								break;
							}
						}
					...
					foreach($packet->inputJson as $arg) {
						$fix = explode(" ", $arg, 20);
						if (count($fix) > 15){
							$this->server->getLogger()->notice("The player $this->username tried to exploit the vulnerability.");
							$this->server->getNetwork()->blockAddress($this->getAddress(), 1500);
							break;
						}
						if (!is_object($arg)){
							$commandText .= " " . $arg;
			...
			case ProtocolInfo::TEXT_PACKET:
				...
				if($packet->type === TextPacket::TYPE_CHAT){
					...
					$maxTotalLength = $this->messageCounter * (512 * 4 + 1);
					if(strlen($packet->message) > $maxTotalLength){
						break;
					}

					$packet->message = TextFormat::clean($packet->message, $this->removeFormat);
					foreach(explode("\n", $packet->message, $this->messageCounter + 1) as $message){
						if(trim($message) !== "" && strlen($message) <= 512 * 4 && mb_strlen($message, 'UTF-8') <= 512 && $this->messageCounter-- > 0){
							if(strpos($message, './') === 0){
			...
			case ProtocolInfo::CRAFTING_EVENT_PACKET:
				...
				if($this->craftingType === self::CRAFTING_ANVIL){
					...
					if($recipe === null){
						...
						}elseif($packet->output[0]->getId() > 0 && $packet->output[1] > 0){
	...
	public function isValidMessage(string $string) : bool{
		if (in_array(strtolower(mb_internal_encoding()), array('utf-8', 'ascii'))) {
			return preg_match('/^[a-zA-Zа-яА-Я0-9\s.,!@#$%^&*()-_=+{}\[\]:";\'<>?\/\\\\]+$/u', $string);
		}
		return false;
	}
	...
	public function sendTitle($title, $subtitle = "", $fadein = 20, $fadeout = 20, $duration = 5) : void{
		$this->addTitle($title, $subtitle, $fadein, $duration, $fadeout);
	}
	public final function close(string $message = "", string $reason = "generic reason", bool $notify = true){
		if($this->isConnected() and !$this->closed){
			...
			if($this->spawned){
				...
				if($this->username != "" and $ev->getQuitMessage() != ""){
