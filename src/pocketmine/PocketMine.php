...
namespace pocketmine {
	...
	const NAME = "Phantomarine";
	const VERSION = "1.1.X";
	const API_VERSION = "3.0.2";
	const CODENAME = "vk.com/phantomarine";
	const CORE_VERSION = '1.1.5-beta';
	...
	if(!class_exists("ClassLoader", false)){
		if(!is_file(\pocketmine\PATH . "src/spl/ClassLoader.php")){
			echo "[CRITICAL] Unable to find the Phantomarine-SPL library." . PHP_EOL;
	...
	if(!flock(\pocketmine\LOCK_FILE, LOCK_EX | LOCK_NB)){
		...
		echo "[CRITICAL] Another Phantomarine instance (PID $pid) is already using this folder (" . realpath(\pocketmine\DATA) . ")." . PHP_EOL;
	...
	if(extension_loaded("pocketmine")){
		$logger->critical("The native extension is no longer supported.");
		++$errors;
	}
