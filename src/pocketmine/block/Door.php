...
abstract class Door extends Transparent implements ElectricalAppliance{
	...
	public function onNearbyBlockChange() : void{
		if ($this->getSide(Vector3::SIDE_DOWN)->getId() === self::AIR) {
			$this->getLevel()->setBlock($this, BlockFactory::get(BlockIds::AIR), false, false);
			if ($this->getSide(Vector3::SIDE_UP) instanceof Door) {
				$this->getLevel()->setBlock($this->getSide(Vector3::SIDE_UP), BlockFactory::get(BlockIds::AIR), false);
			}
	...
	public function place(Item $item, Block $block, Block $target, $face, $fx, $fy, $fz, Player $player = null){
		if($face === 1){
			...
			$this->getLevel()->setBlock($blockUp, BlockFactory::get($this->getId(), $metaUp), true);
			return true;
		}
